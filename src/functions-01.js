...
function convertToDecimal(percent) {
  const numberPart = percent.slice(0, -1);

  return parseFloat(numberPart) / 100;
}
...
function checkSameSum(a1, a2) {
  const sumA1 = a1.reduce((acc, current) => acc + current, 0);
  const sumA2 = a2.reduce((acc, current) => acc + current, 0);
  return sumA1 === sumA2;
}
